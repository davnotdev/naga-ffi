...
pub fn spv_back_capability_set_to_ffi(
    capability_set: &naga::FastHashSet<naga::back::spv::Capability>,
) -> ffi::SPVBackCapabilitySet {
    let capability_set = capability_set.iter().copied().collect::<Vec<_>>();
    ffi::SPVBackCapabilitySet {
        capabilities: unsafe {
            slice_to_ffi(&capability_set, |capability| {
                spv_back_capability_to_ffi(capability)
            })
        },
        capabilities_len: capability_set.len(),
    }
}

pub fn spv_back_writer_flags_to_ffi(
    flags: naga::back::spv::WriterFlags,
) -> ffi::SPVBackWriterFlags {
    let mut result: ffi::SPVBackWriterFlags = 0;
    if flags.contains(naga::back::spv::WriterFlags::DEBUG) {
        result |= ffi::SPVBackWriterFlags_SPVBackWriterFlags_DEBUG;
    }
    if flags.contains(naga::back::spv::WriterFlags::ADJUST_COORDINATE_SPACE) {
        result |= ffi::SPVBackWriterFlags_SPVBackWriterFlags_ADJUST_COORDINATE_SPACE;
    }
    if flags.contains(naga::back::spv::WriterFlags::LABEL_VARYINGS) {
        result |= ffi::SPVBackWriterFlags_SPVBackWriterFlags_LABEL_VARYINGS;
    }
    if flags.contains(naga::back::spv::WriterFlags::FORCE_POINT_SIZE) {
        result |= ffi::SPVBackWriterFlags_SPVBackWriterFlags_FORCE_POINT_SIZE;
    }
    if flags.contains(naga::back::spv::WriterFlags::CLAMP_FRAG_DEPTH) {
        result |= ffi::SPVBackWriterFlags_SPVBackWriterFlags_CLAMP_FRAG_DEPTH;
    }
    if flags.contains(naga::back::spv::WriterFlags::PRINT_ON_RAY_QUERY_INITIALIZATION_FAIL) {
        result |= ffi::SPVBackWriterFlags_SPVBackWriterFlags_PRINT_ON_RAY_QUERY_INITIALIZATION_FAIL;
    }
    sa::const_assert_eq!(
...
pub fn spv_back_binding_info_to_ffi(
    info: &naga::back::spv::BindingInfo,
) -> ffi::SPVBackBindingInfo {
    ffi::SPVBackBindingInfo {
        descriptor_set: info.descriptor_set,
        binding: info.binding,
        binding_array_size: ffi::SPVBackBindingInfo__bindgen_ty_1 {
            some: bool_to_ffi(info.binding_array_size.is_some()),
            value: info.binding_array_size.unwrap_or_default(),
        },
    }
}

pub fn spv_back_binding_map_to_ffi(map: &naga::back::spv::BindingMap) -> ffi::SPVBackBindingMap {
    let entries = map.iter().collect::<Vec<_>>();
    ffi::SPVBackBindingMap {
        entries: unsafe {
            slice_to_ffi(&entries, |(key, value)| ffi::SPVBackBindingMapEntry {
                key: resource_binding_to_ffi(key),
                value: spv_back_binding_info_to_ffi(value),
            })
        },
...
pub fn spv_back_zero_initialize_workgroup_memory_mode_to_ffi(
    mode: &naga::back::spv::ZeroInitializeWorkgroupMemoryMode,
) -> ffi::SPVBackZeroInitializeWorkgroupMemoryMode {
    match mode {
        naga::back::spv::ZeroInitializeWorkgroupMemoryMode::Native => ffi::SPVBackZeroInitializeWorkgroupMemoryMode_SPVBackZeroInitializeWorkgroupMemoryMode_Native,
        naga::back::spv::ZeroInitializeWorkgroupMemoryMode::Polyfill => ffi::SPVBackZeroInitializeWorkgroupMemoryMode_SPVBackZeroInitializeWorkgroupMemoryMode_Polyfill,
        naga::back::spv::ZeroInitializeWorkgroupMemoryMode::None => ffi::SPVBackZeroInitializeWorkgroupMemoryMode_SPVBackZeroInitializeWorkgroupMemoryMode_None,
    }
}

pub fn spv_back_source_language_to_ffi(
    lang: &naga::back::spv::SourceLanguage,
) -> ffi::SPVBackSourceLanguage {
    match lang {
...
pub fn spv_back_debug_info_to_ffi(
    debug_info: &naga::back::spv::DebugInfo,
) -> ffi::SPVBackDebugInfo {
    ffi::SPVBackDebugInfo {
        source_code: unsafe { string_to_ffi(debug_info.source_code) },
        file_name: unsafe { string_to_ffi(debug_info.source_code) },
        language: spv_back_source_language_to_ffi(&debug_info.language),
    }
}

pub fn spv_back_options_to_ffi(options: &naga::back::spv::Options) -> ffi::SPVBackOptions {
    ffi::SPVBackOptions {
        lang_version: [options.lang_version.0, options.lang_version.1],
        flags: spv_back_writer_flags_to_ffi(options.flags),
        fake_missing_bindings: bool_to_ffi(options.fake_missing_bindings),
        binding_map: spv_back_binding_map_to_ffi(&options.binding_map),
        capabilities: ffi::SPVBackOptions__bindgen_ty_1 {
            some: bool_to_ffi(options.capabilities.is_some()),
            value: options
                .capabilities
                .as_ref()
                .map(spv_back_capability_set_to_ffi)
                .unwrap_or_default(),
        },
        bounds_check_policies: bound_check_policies_to_ffi(&options.bounds_check_policies),
        zero_initialize_workgroup_memory: spv_back_zero_initialize_workgroup_memory_mode_to_ffi(
            &options.zero_initialize_workgroup_memory,
        ),
        force_loop_bounding: bool_to_ffi(options.force_loop_bounding),
        ray_query_initialization_tracking: bool_to_ffi(options.ray_query_initialization_tracking),
        use_storage_input_output_16: bool_to_ffi(options.use_storage_input_output_16),
        debug_info: ffi::SPVBackOptions__bindgen_ty_2 {
            some: bool_to_ffi(options.debug_info.is_some()),
            value: options
                .debug_info
                .as_ref()
                .map(|d| spv_back_debug_info_to_ffi(d))
                .unwrap_or_default(),
        },
    }
}

pub fn spv_back_pipeline_options_to_ffi(
    options: &naga::back::spv::PipelineOptions,
) -> ffi::SPVBackPipelineOptions {
    ffi::SPVBackPipelineOptions {
